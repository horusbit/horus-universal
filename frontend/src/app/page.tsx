...
export default function HorusChat() {
  ...
  const exportAsPDF = useCallback(() => {
    ...
    const rows = messages.map(m => {
      ...
      const name = isUser ? "Tu" : (m.agent || "atlas").toUpperCase();
      const avatar = isUser ? "U" : "H";
      ...
      return (
        ...
        '<div style="font-size:11px;color:#64748b;margin-bottom:8px;font-weight:600">' + avatar + ' ' + name + '</div>' +
    ...
    const html = (
      '<!DOCTYPE html><html><head><meta charset="utf-8"><title>HORUS</title>' +
      '<style>body{font-family:sans-serif;background:#0a0a0f;color:#e2e8f0;margin:0;padding:24px}' +
      ...
      '<h1 style="font-size:20px;margin:0 0 4px">HORUS Universal</h1>' +
      '<div style="font-size:11px;color:#64748b;margin-bottom:24px">Exportado el ' + new Date().toLocaleString("es-ES") + '</div>' +
      rows +
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      ...
      const msg = err instanceof Error && err.message.includes("limit_reached")
        ? "Limite diario alcanzado. Actualiza a Pro para uso ilimitado."
        : "Error conectando con el servidor. Verifica que el backend este corriendo.";
      setMessages(prev =>
        prev.map(m =>
    ...
    <div className="flex h-screen bg-[#0a0a0f] overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <header className="flex items-center gap-3 px-4 py-3 border-b border-[#1e1e2e] bg-[#12121a] flex-shrink-0">
          ...
          <a
            href="/teams"
            className="text-xs text-[#64748b] hover:text-indigo-400 transition-colors flex-shrink-0"
            title="Mis equipos"
          >
            👥
          </a>
        ...
        <div className="px-4 pb-4 pt-2 border-t border-[#1e1e2e] bg-[#0a0a0f] flex-shrink-0">
          {serverWaking && (
            <div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-900/20 border border-amber-500/20 rounded-lg px-3 py-2 mb-2">
              ...
              Despertando servidor... (primera vez tarda unos segundos)
            </div>
          )}
          <ChatInput
            onSend={handleSend}
